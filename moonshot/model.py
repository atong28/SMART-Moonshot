...
from settings import MoonshotArgs
...
class Moonshot(pl.LightningModule):
    def __init__(
        ...
        output_dims['y'] = 0               # no y‐diffusion target

        self.args = args
        self.name = args.name
        self.model_dtype = torch.float32
        self.T = args.diffusion_steps
        self.val_num_samples = args.val_samples_to_generate
        self.test_num_samples = args.test_samples_to_generate
        ...
        self.noise_schedule = PredefinedNoiseScheduleDiscrete(args.diffusion_noise_schedule, timesteps=args.diffusion_steps)
        self.denoise_nodes = args.denoise_nodes

        if args.transition == 'uniform':
        ...
        self.save_hyperparameters(ignore=['train_metrics', 'sampling_metrics', 'spectre'])
    ...
    def training_step(self, data, i):
        ...
        noisy_data = self.apply_noise(X, E, data.y, node_mask)
        with torch.no_grad():
            spec_emb = self.spectre(spec_inputs)   # → Tensor(bs, spec_dim)
    ...
    def configure_optimizers(self):
        ...
        if self.args.scheduler == 'const':
            return torch.optim.AdamW(trainable, lr=self.args.lr, amsgrad=True, weight_decay=self.args.weight_decay)
        elif self.args.scheduler == 'one_cycle':
            opt = torch.optim.AdamW(trainable, lr=self.args.lr, amsgrad=True, weight_decay=self.args.weight_decay)
    ...
    def validation_step(self, data, i):
        ...
        spec_emb = self.spectre(spec_inputs)
        extra_data = self.compute_extra_data(noisy_data, spec_emb)
        ...
        nll = self.compute_val_loss(pred, noisy_data, spec_emb, dense_data.X, dense_data.E, data.y,  node_mask, test=False)
        ...
        if self.val_counter % self.args.sample_every_val == 0:
    ...
    def test_step(self, data, i):
        ...
        nll = self.compute_val_loss(pred, noisy_data, spec_emb, dense_data.X, dense_data.E, data.y,  node_mask, test=True)
    ...
    def reconstruction_logp(self, t, X, E, y, node_mask, spec_emb):
    ...
    def compute_val_loss(self, pred, noisy_data, spec_emb, X, E, y, node_mask, test=False):
       ...
       """
        ...
        prob0 = self.reconstruction_logp(t, X, E, y, node_mask, spec_emb)
    ...
    def compute_extra_data(self, noisy_data, spec_emb):
        ...
        # ── now inject the per‐node SPECTRE embedding
        extra_y = torch.cat((extra_y, spec_emb), dim=-1)
