...
OUTPUT_DIR = Path('results/analysis/np_pathway_top20_trial1')
TOP_K_CLASSES = 20
BATCH_SIZE = 64
...
# Helpers
# ======================
def get_np_pathway_label(datamodule, idx):
    labels = datamodule.test.data[idx][1].get('np_pathway', [])
...
def collect_test_cls_embeddings(model, datamodule, batch_size=64, device=None):
    ...
    with torch.no_grad():
        ...
        while i < N:
            ...
            for k in range(i, j):
                y_types_vals.append(get_np_pathway_label(datamodule, k))
            i = j
...
def plot_density_clouds_single(Z, labels, classes, out_png=None,
    ...
    ax.set_title("Top-20 NP pathways — UMAP density clouds (Trial 1)", fontsize=12, pad=10)
    if legend_handles:
...
def plot_points_umap_single(
    ...
    ax.set_xticks([]); ax.set_yticks([])
    ax.set_title("Top-20 NP pathways — UMAP points (Trial 1)", fontsize=12, pad=10)
