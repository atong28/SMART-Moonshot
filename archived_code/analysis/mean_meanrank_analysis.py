#!/usr/bin/env python3
"""
Mean Rank per NP superclass (Top-20), using your cm(...) + RankingSet.

Outputs:
  - results/analysis/meanrank_by_class_trial1/per_item_meanrank.csv
  - results/analysis/meanrank_by_class_trial1/per_class_meanrank.csv
  - results/analysis/meanrank_by_class_trial1/top20_meanrank_bar.png
"""
...
# CONFIG
# ======================
CKPT = 'old_ckpt/trial_1.ckpt'
DATA_ROOT = '/data/nas-gpu/wang/atong/MoonshotDataset'
INPUT_TYPES = ['hsqc', 'h_nmr', 'c_nmr', 'mw']

OUTPUT_DIR = Path('results/analysis/meanrank_by_class_trial1')
BATCH_SIZE = 128
TOP_K = 20
INCLUDE_MULTI = False
...
@torch.no_grad()
def collect_meanrank_per_item(model, dm, device):
    ranker = model.ranker
    per_item = []  # rows: idx, class, mean_rank (per-sample if available)
    i, N = 0, len(dm.test)
    while i < N:
        ...
        # We want a per-sample "mean_rank".
        # If rank_res are numeric ranks, use them directly.
        # If rank_res are boolean hits (rare), fall back to batch mean (same value for all rows).
        if isinstance(rank_res, torch.Tensor):
            rr = rank_res.detach().cpu()
            if rr.dtype == torch.bool:
                # No numeric ranks available; use batch mean as fallback
                per_sample_meanrank = np.full(rr.shape[0], float(metrics['mean_rank']), dtype=np.float32)
            else:
                per_sample_meanrank = rr.numpy().astype(np.float32)
        else:
            arr = np.array(rank_res)
            if arr.dtype == np.bool_:
                per_sample_meanrank = np.full(arr.shape[0], float(metrics['mean_rank']), dtype=np.float32)
            else:
                per_sample_meanrank = arr.astype(np.float32)
        ...
        for r, idx in enumerate(range(i, j)):
            ...
            if ltype == 'single':
                ...
                cls = 'Unknown'
            per_item.append({"idx": idx, "class": cls, "mean_rank": float(per_sample_meanrank[r])})
        i = j

    return per_item

def summarize_meanrank(per_item):
    counts = Counter([r['class'] for r in per_item])
    order = [c for c,_ in counts.most_common()]
    stats = []
    for c in order:
        vals = [r['mean_rank'] for r in per_item if r['class']==c]
...
def save_csv_per_item(per_item, path):
    with open(path, 'w') as f:
        f.write("idx,class,mean_rank\n")
        for r in per_item:
            f.write(f"{r['idx']},{r['class']},{r['mean_rank']:.6f}\n")
...
def plot_top20_meanrank(stats, top_k, out_png, dpi=260):
    ...
    # For display, sort ascending (lower mean rank is better)
    top = sorted(top, key=lambda d: d['mean_meanrank'])
    ...
    plt.xlabel("Mean of per-sample rank (lower is better)")
...
def write_web_csv(stats, out_path: Path):
    """
    stats: list of dicts from summarize_meanrank()
           keys: {'class','n','mean_meanrank', ...}
    ...
    with open(out_path, "w", newline="") as f:
        ...
        for s in stats:
            w.writerow([s["class"], float(s["mean_meanrank"]), int(s["n"])])
...
def main():
    ...
    print("[INFO] Evaluating Mean Rank per item…")
    per_item = collect_meanrank_per_item(model, dm, device)
    if not per_item:
        print("[WARN] No items found (check INCLUDE_MULTI/INCLUDE_UNKNOWN).")
        return

    # CSVs
    per_item_csv = OUTPUT_DIR / "per_item_meanrank.csv"
    save_csv_per_item(per_item, per_item_csv)
    stats = summarize_meanrank(per_item)
    per_class_csv = OUTPUT_DIR / "per_class_meanrank.csv"
    save_csv_per_class(stats, per_class_csv)
    write_web_csv(stats, Path("./acc_meanrank.csv"))
