...
OUTPUT_DIR = Path('results/analysis/np_superclass_top20_trial1_points_small_peptides_highlighted')
TOP_K_CLASSES = 20
BATCH_SIZE = 64

# Highlighting
HIGHLIGHT_CLASS = 'Small peptides'  # e.g., "Alkaloids" to highlight; None to disable
FADE_ALPHA = 0.15       # alpha for non-highlighted classes
...
def plot_points_umap_single(
    ...
    for j, cls in enumerate(classes):
        ...
        if pts.size == 0:
            continue
        # Choose alpha
        if highlight_class is not None and cls != highlight_class:
            alpha = fade_alpha
        else:
            alpha = point_alpha
        ax.scatter(
    ...
    ncol = 1  # you can still make this conditional if you want
    ax.legend(
        handles=handles,
        loc='center left',           # anchor to left-center of legend box
...
def main():
    ...
    plot_points_umap_single(
        ...
        point_size=POINT_SIZE, point_alpha=POINT_ALPHA, max_per_class=MAX_PER_CLASS,
        palette="tab20", shuffle=True, rasterized_pdf=True, dpi=PNG_DPI,
        title="Top-20 NP superclasses — UMAP points (Tanimoto, Trial 1)",
        highlight_class=HIGHLIGHT_CLASS,
        fade_alpha=FADE_ALPHA
    )
