#!/usr/bin/env python3
"""
Rank-1 accuracy per NP superclass (Top-20), averaged across multiple checkpoints.

Outputs:
  - results/analysis/rank1_by_class_ensemble/per_item_rank1_ensemble.csv
  - results/analysis/rank1_by_class_ensemble/per_class_rank1_ensemble.csv
  - results/analysis/rank1_by_class_ensemble/top20_rank1_bar.png
"""
...
OUTPUT_DIR = Path('results/analysis/rank1_by_class_ensemble')
BATCH_SIZE = 128
TOP_K = 20
INCLUDE_MULTI = False
...
@torch.no_grad()
def collect_rank1_per_item_single_model(model, dm, device):
    ...
    per_item = []  # list of dicts for this model
    i, N = 0, len(dm.test)
    while i < N:
        ...
        # normalize to 0/1 hit
        if isinstance(rank_res, torch.Tensor):
            rr = rank_res.detach().cpu()
            hits = (rr < 1).numpy().astype(int) if rr.dtype != torch.bool else rr.numpy().astype(int)
        else:
            arr = np.array(rank_res)
            hits = (arr < 1).astype(int) if arr.dtype != np.bool_ else arr.astype(int)
...
def average_per_item_across_models(per_item_lists):
    """
    Merge by (idx, class), average rank1 across models.
    Returns list of dicts: {idx, class, rank1_mean}.
    """
    bucket = defaultdict(list)
    for lst in per_item_lists:
        for row in lst:
            bucket[(row["idx"], row["class"])].append(row["rank1"])
    out = []
    for (idx, cls), vals in bucket.items():
        out.append({"idx": idx, "class": cls, "rank1_mean": float(np.mean(vals))})
    return out

def summarize_rank1(per_item_avg):
    counts = Counter([r['class'] for r in per_item_avg])
    order = [c for c,_ in counts.most_common()]
    stats = []
    for c in order:
        vals = [r['rank1_mean'] for r in per_item_avg if r['class']==c]
...
def save_csv_per_item(per_item_avg, path):
    with open(path, 'w') as f:
        f.write("idx,class,rank1_mean\n")
        for r in per_item_avg:
            f.write(f"{r['idx']},{r['class']},{r['rank1_mean']:.6f}\n")
...
def plot_top20_rank1(stats, top_k, out_png, dpi=260):
    ...
    plt.xlim(0, 1.0)
    plt.xlabel("Mean Rank-1 (averaged across models)")
...
def main():
    ...
    for ck in CKPTS:
        ...
        per_item = collect_rank1_per_item_single_model(model, dm, device)
        per_model_lists.append(per_item)

    # average per-item across models
    per_item_avg = average_per_item_across_models(per_model_lists)
    if not per_item_avg:
        print("[WARN] No items after filtering.")
        return

    # save per-item & per-class
    per_item_csv = OUTPUT_DIR / "per_item_rank1_ensemble.csv"
    save_csv_per_item(per_item_avg, per_item_csv)

    stats = summarize_rank1(per_item_avg)
    per_class_csv = OUTPUT_DIR / "per_class_rank1_ensemble.csv"
