...
def sample_discrete_features(probX, probE, node_mask):
    ...
    return GraphData(X=X_t, E=E_t, y=torch.zeros(bs, 0).type_as(X_t))
...
def posterior_distributions(X, E, X_t, E_t, Qt, Qsb, Qtb):
    """Compute posterior distributions over X and E given their noisy versions.

    The returned GraphData's y field is left empty, since global features are
    not used in the KL computation.
    """
    ...
    empty_y = prob_X.new_zeros(prob_X.size(0), 0)
    return GraphData(X=prob_X, E=prob_E, y=empty_y)
...
def sample_discrete_feature_noise(limit_dist, node_mask):
    ...
    return GraphData(X=U_X, E=U_E, y=U_y).mask(node_mask)
