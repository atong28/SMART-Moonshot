...
from ..core.const import ATOM_TO_WEIGHT, VALENCY
...
class ExtraMolecularFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        return GraphData(X=torch.cat((charge, valency), dim=-1), E=extra_edge_attr, y=weight)
...
class ChargeFeature:
    ...
    def __call__(self, noisy_data):
        ...
        valencies = torch.tensor(VALENCY, device=noisy_data['X_t'].device).reshape(1, 1, -1)
...
class WeightFeature:
    def __init__(self):
        self.max_weight = max(ATOM_TO_WEIGHT.values())
        self.atom_weight_list = torch.tensor(list(ATOM_TO_WEIGHT.values()))
