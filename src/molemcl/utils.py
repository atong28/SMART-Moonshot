...
class MaskAtomBalanced(MaskAtom):
    def __init__(self, num_atom_type, num_edge_type, mask_rate, class_probs, mask_edge=True):
        super().__init__(num_atom_type, num_edge_type, mask_rate, mask_edge)
        self.class_probs = class_probs  # tensor [num_atom_type]

    def __call__(self, data, masked_atom_indices=None):
        if masked_atom_indices is None:
            n = data.x.size(0)
            k = max(1, int(n * self.mask_rate))
            types = data.x[:, 0]
            p = self.class_probs[types].clamp_min(1e-12)
            p = p / p.sum()
            masked_atom_indices = torch.multinomial(p, k, replacement=False).tolist()
        return super().__call__(data, masked_atom_indices)