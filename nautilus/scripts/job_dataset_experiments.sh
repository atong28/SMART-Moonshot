...
for dataset in MARINAControl1 MARINADataset1 MARINADataset2 MARINADataset3 MARINADataset4; do
  for seed in 0 1 2; do
    ...
    sed "39s|.*|              bash /root/gurusmart/startup.sh ${dataset}.zip \&\& |" "$yaml" > "$yaml.tmp" && mv "$yaml.tmp" "$yaml"
    sed "41s|.*|              pixi run train.marina --input_types ${input_types} --seed ${seed} --experiment_name ${experiment_name} |" "$yaml" > "$yaml.tmp" && mv "$yaml.tmp" "$yaml"
